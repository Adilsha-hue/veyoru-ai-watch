...
static lv_obj_t *makeMetricRing(lv_obj_t *parent, int x, const char *caption, int value) {
...
static void reportUiState() {
  ...
  Serial.printf(
      ...
      "\"status\":\"%s\",\"hr\":%d,\"battery\":%d,\"power\":\"%s\",\"voltage_mv\":%d,\"accent\":\"%s\"}\n",
      screen.c_str(), title.c_str(), body.c_str(), status.c_str(),
      uiHeartRate, uiBattery, jsonEscape(powerLabel).c_str(), uiVoltageMv, accentHex);
}

static void refreshUiFromState() {
  bool assistant = uiScreen == "assistant" || uiScreen == "result";
  if (assistant) {
...
static void runOfflineAssistant(const String &line) {
  ...
  AssistantReply reply;
  if (!handledOffline) reply = cloudReply(prompt, priorContext);
